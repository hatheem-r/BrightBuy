  ...
  const fetchCustomerOrders = async () => {
    try {
      ...
      const stats = {
        ...
        totalSpent: customerOrders.reduce((sum, o) => sum + parseFloat(o.total || 0), 0),
      };
      setOrderStats(stats);
    } catch (error) {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-6">
            ...
            <p className="text-3xl font-bold text-purple-700 dark:text-purple-300">Rs. {orderStats.totalSpent.toLocaleString()}</p>
          </div>
        </div>
        ...
        {filteredOrders.length === 0 ? (
          <div className="bg-card border border-card-border rounded-lg shadow-sm p-12 text-center">
            ...
            {filterStatus === "all" && (
              <Link href="/" className="inline-block px-6 py-3 bg-primary text-white rounded-lg hover:opacity-90 font-medium">
                Start Shopping
              </Link>
            )}
          </div>
              ...
              <div key={order.order_id} className="bg-card border border-card-border rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                ...
                {order.payment_status === "pending" && (
                  <div className="mt-4 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-3 flex items-center text-sm text-orange-800 dark:text-orange-300">
                    ...
                    <span>Payment pending - Please complete payment to process your order</span>
                  </div>
                )}
              </div>
            ))}
