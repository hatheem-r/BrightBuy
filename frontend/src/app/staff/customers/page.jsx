...
export default function CustomerManagement() {
  ...
  const fetchCustomers = async () => {
    try {
      const response = await fetch("http://localhost:5001/api/staff/customers", {
        headers: {
  ...
  const fetchCustomerDetails = async (customerId) => {
    try {
      const response = await fetch(`http://localhost:5001/api/staff/customers/${customerId}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        },
      });
      ...
      <div className="container mx-auto px-6 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {selectedCustomer.addresses && selectedCustomer.addresses.length > 0 && (
                <div className="md:col-span-2">
                  <h3 className="font-semibold text-text-primary mb-3">Addresses</h3>
        ...
        <div className="bg-card border border-card-border rounded-lg overflow-hidden">
          <div className="px-6 py-4 bg-gray-50 dark:bg-gray-800 border-b border-card-border">
            <h2 className="text-xl font-bold text-text-primary">All Customers</h2>
            <p className="text-sm text-text-secondary">Total: {filteredCustomers.length} customers</p>
          </div>
                  ...
                  filteredCustomers.map((customer) => (
                    <tr key={customer.customer_id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                      <td className="px-6 py-4">
                        ...
                        <div className="text-xs text-text-secondary">ID: {customer.customer_id}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-text-primary">{customer.email}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-text-primary">{customer.phone || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
