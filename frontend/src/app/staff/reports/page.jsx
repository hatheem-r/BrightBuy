...
export default function StaffReports() {
  ...
  useEffect(() => {
    ...
    const fetchHelperData = async () => {
      ...
      try {
        ...
        const yearsRes = await fetch("http://localhost:5001/api/reports/available-years", {
        ...
        const citiesRes = await fetch("http://localhost:5001/api/reports/available-cities", {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (citiesRes.ok) {
  ...
  useEffect(() => {
    ...
    fetchReportData();
  }, [activeReport, user, dateRange, selectedYear, inventoryFilter, selectedCity]);

  // API Fetch Functions
  const fetchSalesSummary = async (token) => {
    const res = await fetch(
      `http://localhost:5001/api/reports/sales-summary?startDate=${dateRange.startDate}&endDate=${dateRange.endDate}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (res.ok) {
  ...
  const fetchTopProducts = async (token) => {
    const res = await fetch(
      `http://localhost:5001/api/reports/top-selling-products?startDate=${dateRange.startDate}&endDate=${dateRange.endDate}&limit=10`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (res.ok) {
      const data = await res.json();
  ...
  const fetchQuarterlySales = async (token) => {
    const res = await fetch(
      `http://localhost:5001/api/reports/quarterly-sales?year=${selectedYear}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (res.ok) {
      const data = await res.json();
  ...
  const fetchCustomerSummary = async (token) => {
    const res = await fetch(
      "http://localhost:5001/api/reports/customer-order-summary?limit=50&minOrders=1",
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (res.ok) {
      const data = await res.json();
  ...
  const fetchInventoryReport = async (token) => {
    ...
    const res = await fetch(
      `http://localhost:5001/api/reports/inventory?limit=100${statusParam}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (res.ok) {
      const data = await res.json();
  ...
  const fetchInventoryUpdates = async (token) => {
    const res = await fetch(
      "http://localhost:5001/api/reports/inventory-updates?days=30&limit=50",
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (res.ok) {
      const data = await res.json();
  ...
  const fetchDeliveryEstimates = async (token) => {
    ...
    const res = await fetch(
      `http://localhost:5001/api/reports/order-delivery-estimate?limit=50${cityParam}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (res.ok) {
      const data = await res.json();
          ...
          {loading ? (
            ...
            <>
                    ...
                    <>
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                        ...
                        <div className="bg-green-50 p-4 rounded-lg">
                          ...
                          <p className="text-2xl font-bold text-green-600">
                            ${parseFloat(salesSummary.sales.total_revenue || 0).toFixed(2)}
                          </p>
                        </div>
                        ...
                        <div className="bg-orange-50 p-4 rounded-lg">
                          ...
                          <p className="text-2xl font-bold text-orange-600">
                            ${parseFloat(salesSummary.sales.avg_order_value || 0).toFixed(2)}
                          </p>
                        </div>
                      </div>
                      ...
                      {salesSummary.topCategory && (
                        <div className="bg-gray-50 p-4 rounded-lg mb-6">
                          <p className="text-sm text-gray-600 mb-1">Top Performing Category</p>
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div className="grid grid-cols-4 gap-4">
                          <div>
                            ...
                            <p className="text-lg font-bold">{salesSummary.inventory.total_variants}</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-600">In Stock</p>
                            <p className="text-lg font-bold text-green-600">{salesSummary.inventory.in_stock}</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-600">Low Stock</p>
                            <p className="text-lg font-bold text-orange-600">{salesSummary.inventory.low_stock}</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-600">Out of Stock</p>
                            <p className="text-lg font-bold text-red-600">{salesSummary.inventory.out_of_stock}</p>
                          </div>
                        </div>
                      </div>
                    </>
                    ...
                    <table className="min-w-full">
                      <thead className="bg-gray-100">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Total Sales</th>
                        </tr>
                      </thead>
                        ...
                        {topProducts.length > 0 ? (
                          topProducts.map((product, index) => (
                            <tr key={product.product_id} className="hover:bg-gray-50">
                              ...
                              <td className="px-4 py-3 text-sm text-gray-700">{product.product_id}</td>
                              <td className="px-4 py-3 text-sm font-medium text-gray-900">{product.product_name}</td>
                              <td className="px-4 py-3 text-sm text-gray-700">{product.total_quantity_sold}</td>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {quarterlySales.length > 0 ? (
                      quarterlySales.map((quarter) => (
                        <div key={quarter.quarter} className="bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-lg">
                          <h3 className="text-lg font-bold text-gray-900 mb-2">Q{quarter.quarter} {selectedYear}</h3>
                          <div className="space-y-2">
                            ...
                            <div>
                              ...
                              <p className="text-lg font-semibold text-gray-900">{quarter.total_orders}</p>
                            </div>
                          </div>
                        </div>
                      ))
                    ...
                    <table className="min-w-full">
                      <thead className="bg-gray-100">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Avg Order Value</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-200">
                        {customerSummary.length > 0 ? (
                          customerSummary.map((customer) => (
                            <tr key={customer.customer_id} className="hover:bg-gray-50">
                              <td className="px-4 py-3 text-sm text-gray-700">{customer.customer_id}</td>
                              <td className="px-4 py-3 text-sm font-medium text-gray-900">{customer.customer_name}</td>
                              <td className="px-4 py-3 text-sm text-gray-700">{customer.total_orders}</td>
                              <td className="px-4 py-3 text-sm font-semibold text-green-600">
                                ${parseFloat(customer.total_spent || 0).toFixed(2)}
                              </td>
                              <td className="px-4 py-3 text-sm text-gray-700">
                                ${(parseFloat(customer.total_spent || 0) / customer.total_orders).toFixed(2)}
                              </td>
                            </tr>
                          ))
                        ) : (
              ...
              {activeReport === "inventory-report" && (
                <div>
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-gray-900">Inventory Status Report</h2>
                    <select
                    ...
                    <table className="min-w-full">
                      <thead className="bg-gray-100">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Color/Size</th>
                        ...
                        {inventoryReport.length > 0 ? (
                          inventoryReport.map((item) => (
                            <tr key={item.variant_id} className="hover:bg-gray-50">
                              ...
                              <td className="px-4 py-3 text-sm font-semibold">{item.stock}</td>
                              <td className="px-4 py-3">
              ...
              {activeReport === "category-orders" && (
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 mb-6">Orders by Category</h2>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {categoryOrders.length > 0 ? (
                      categoryOrders.map((category) => (
                        <div key={category.category_id} className="bg-gradient-to-br from-purple-50 to-purple-100 p-5 rounded-lg">
                          <h3 className="text-lg font-bold text-gray-900 mb-2">{category.category_name}</h3>
                          <p className="text-3xl font-bold text-purple-600">{category.total_orders}</p>
                          <p className="text-sm text-gray-600">Total Orders</p>
                        </div>
                      ))
                    ) : (
              ...
              {activeReport === "inventory-updates" && (
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 mb-6">Recent Inventory Updates (Last 30 Days)</h2>
                    ...
                    <table className="min-w-full">
                      <thead className="bg-gray-100">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Product</th>
                          <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">SKU</th>
                          <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Old Qty</th>
                        ...
                        {inventoryUpdates.length > 0 ? (
                          inventoryUpdates.map((update) => (
                            <tr key={update.update_id} className="hover:bg-gray-50">
                              ...
                              <td className="px-4 py-3 text-sm text-gray-700">{update.staff_name}</td>
                              <td className="px-4 py-3 text-sm font-medium text-gray-900">{update.product_name}</td>
                              <td className="px-4 py-3 text-sm font-mono text-gray-700">{update.sku}</td>
                              <td className="px-4 py-3 text-sm text-gray-700">{update.old_quantity}</td>
                              <td className="px-4 py-3 text-sm">
                                <span className={`font-semibold ${
                                  update.added_quantity > 0 ? 'text-green-600' : 'text-red-600'
                                }`}>
                                  {update.added_quantity > 0 ? '+' : ''}{update.added_quantity}
                                </span>
                              </td>
                              <td className="px-4 py-3 text-sm text-gray-600">{update.note || '-'}</td>
                            </tr>
                          ))
                        ) : (
                          <tr>
              ...
              {activeReport === "delivery-estimates" && (
                <div>
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-gray-900">Order Delivery Estimates</h2>
                    <select
                      value={selectedCity}
                    ...
                    <table className="min-w-full">
                      <thead className="bg-gray-100">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Est. Days</th>
                        ...
                        {deliveryEstimates.length > 0 ? (
                          deliveryEstimates.map((order) => (
                            <tr key={order.order_id} className="hover:bg-gray-50">
                              <td className="px-4 py-3 text-sm font-semibold text-gray-900">{order.order_id}</td>
                              ...
                              <td className="px-4 py-3 text-sm text-gray-700">{order.zip_code || '-'}</td>
                              <td className="px-4 py-3 text-sm text-gray-700">
