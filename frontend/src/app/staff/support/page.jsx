  ...
  const [tickets, setTickets] = useState([
    {
      ...
      description: "I ordered a laptop but haven't received it yet. It's been 5 days since the expected delivery date."
    },
    {
      id: 2,
      ...
      description: "The camera I received has a cracked lens. I need a replacement."
    },
    {
      id: 3,
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
            <div className="text-sm text-yellow-800 dark:text-yellow-300 font-medium">Open Tickets</div>
            <div className="text-2xl font-bold text-yellow-900 dark:text-yellow-100">
              {tickets.filter(t => t.status === 'open').length}
            </div>
          </div>
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
            <div className="text-sm text-blue-800 dark:text-blue-300 font-medium">In Progress</div>
            <div className="text-2xl font-bold text-blue-900 dark:text-blue-100">
              {tickets.filter(t => t.status === 'in_progress').length}
            </div>
          </div>
          <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
            <div className="text-sm text-green-800 dark:text-green-300 font-medium">Resolved</div>
            <div className="text-2xl font-bold text-green-900 dark:text-green-100">
              {tickets.filter(t => t.status === 'resolved').length}
            </div>
          </div>
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
            <div className="text-sm text-red-800 dark:text-red-300 font-medium">High Priority</div>
        ...
        <div className="mb-6 flex gap-2">
          <button
            ...
            className={`px-4 py-2 rounded ${filterStatus === 'all' ? 'bg-primary text-white' : 'bg-gray-200 text-gray-700'}`}
          >
            All
          </button>
          <button
            onClick={() => setFilterStatus('open')}
            className={`px-4 py-2 rounded ${filterStatus === 'open' ? 'bg-primary text-white' : 'bg-gray-200 text-gray-700'}`}
          >
            Open
          </button>
          <button
            onClick={() => setFilterStatus('in_progress')}
            className={`px-4 py-2 rounded ${filterStatus === 'in_progress' ? 'bg-primary text-white' : 'bg-gray-200 text-gray-700'}`}
          >
            In Progress
          </button>
          <button
            onClick={() => setFilterStatus('resolved')}
            className={`px-4 py-2 rounded ${filterStatus === 'resolved' ? 'bg-primary text-white' : 'bg-gray-200 text-gray-700'}`}
          >
            Resolved
          </button>
        </div>
        ...
        <div className="bg-card border border-card-border rounded-lg overflow-hidden">
          <div className="px-6 py-4 bg-gray-50 dark:bg-gray-800 border-b border-card-border">
            <h2 className="text-xl font-bold text-text-primary">Support Tickets</h2>
            <p className="text-sm text-text-secondary">Showing {filteredTickets.length} tickets</p>
          </div>
            ...
            {filteredTickets.length === 0 ? (
              ...
              filteredTickets.map((ticket) => (
                ...
                >
                  ...
                  <p className="text-sm text-text-secondary truncate mb-2">{ticket.description}</p>
